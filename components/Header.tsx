    ...
    <header className="w-full border-b py-4" style={{ background: 'var(--white)', borderColor: 'rgba(214,179,106,0.12)' }}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm" aria-label="Primary">
          {NAV.map((item)=> (
            <a key={item.href} href={item.href} className="text-charcoal hover:text-champagne">{item.label.toUpperCase()}</a>
          ))}

          <a href="/contact" className="ml-6 inline-block px-5 py-2 border border-champagne-strong text-champagne hover:bg-warmChampagne/90 transition" aria-label="Plan your wedding">PLAN YOUR WEDDING</a>
        </nav>
